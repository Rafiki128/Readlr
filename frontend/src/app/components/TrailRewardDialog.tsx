...
export function TrailRewardDialog({ reward, onClose, onBook }: {
  reward: TrailReward; onClose: () => void; onBook: () => void;
}) {
  const next = POINT_STICKERS.find((item) => item.points > reward.totalPoints);
  const reducedMotion = useReducedMotion();
  const previousMilestone = next ? next.points - 500 : 1000;
  const progress = Math.min(100, ((reward.totalPoints - previousMilestone) / 500) * 100);
  return (
    <Dialog open onOpenChange={(open) => { if (!open) onClose(); }}>
      <DialogContent className="trail-reward">
        <header className="trail-reward__header">
          <p className="trail-reward__eyebrow"><Flag size={15} />Trail complete</p>
          <DialogTitle className="trail-reward__title">{reward.earnedPoints ? "A new friend joins you!" : "You did it again!"}</DialogTitle>
          <DialogDescription className="trail-reward__description">{reward.earnedPoints ? "Your voice helped Milo along the trail." : "Thanks for helping Milo practice."}</DialogDescription>
        </header>
        ...
        <section className="trail-reward__reveal" aria-label="Your sticker reward">
          ...
          <p className="trail-reward__saved"><BookOpen size={14} />{reward.earnedPoints ? "Added to your sticker book" : "Already in your sticker book"}</p>
        </section>

        <div className="trail-reward__details">
          <div className="trail-reward__points">
            <span className="trail-reward__point-icon"><Star size={24} fill="currentColor" /></span>
            <div><strong>{reward.earnedPoints ? `+${reward.earnedPoints}` : "Great practice!"}</strong><span>{reward.earnedPoints ? "trail points" : "Your points are safe"}</span></div>
            <div className="trail-reward__total"><strong>{reward.totalPoints.toLocaleString()}</strong><span>points collected</span></div>
          </div>
          ...
          <div className="trail-reward__milestone">
            ...
            <div className="trail-reward__meter" role="progressbar" aria-label="Next bonus sticker" aria-valuemin={0} aria-valuemax={500} aria-valuenow={Math.min(500, reward.totalPoints - previousMilestone)}><motion.div initial={false} animate={{ width: `${progress}%` }} transition={{ duration: reducedMotion ? 0 : 0.6 }} /></div>
            <p className="trail-reward__remaining">{next ? `${next.points - reward.totalPoints} more points to meet them` : "Milo's valley is full of friends."}</p>
          </div>
